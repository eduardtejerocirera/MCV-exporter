  ...
  )
{
  ...
  oUV.x = -( 1 - ( iPos.x * 2.0 )) * CameraTanHalfFov * CameraAspectRatio; 
  oUV.y =  ( 1 - ( iPos.y * 2.0 )) * CameraTanHalfFov; 
...
) : SV_Target
{
  ...
  const float amount_displaced = GlobalFXVal1 / CameraZFar;
  const float zrange_discard = GlobalFXVal2;
  const float amount_normal_z = GlobalFXVal3;
  ...
  for (int i = 0; i < 12; ++i) {
    ...
    float4 sample_proj = mul( float4( sample_view_pos, 1), CameraProjWithOffset );
    sample_proj.xy /= sample_proj.w;

    // Read existing depth at the projected coord
    float prevDepthAtSampleCoords = txGLinearDepth.Sample(samClampLinear, sample_proj.xy).r;
  ...
  return ( 1 - GlobalFXAmount ) + (1 - occlusion / 12 ) * GlobalFXAmount;
}
